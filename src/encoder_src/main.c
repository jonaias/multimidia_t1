...
void print_help(){
	printf("Usage: encode -d -c -h <arquivo.wav> <saida.bin>\n");
	printf("\tencode \t\t nome do codificador\n");
...
int main (int argc, char **argv)
{
	...
	if((dflag+cflag+hflag)&&((argc-optind)==2)){
		...
		if(cflag){
			run_length_encode(buffer,&buffer_size);
		}
		
		if(dflag){
			difference_encode(buffer,&buffer_size);
		}
